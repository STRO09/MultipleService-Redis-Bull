...
function MongoDBData() {
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      ...
      newSocket.emit('requestMongoData');
    });
    ...
    // Handle MongoDB data response
    newSocket.on('mongoDataResponse', (response) => {
      console.log('Received mongoDataResponse:', response);
      setLoading(false);
      ...
      if (response.success) {
        ...
        // ✅ ADD: Log auto-refresh if triggered by bulk upload
        if (response.trigger === 'bulk_upload') {
          console.log('🔄 Auto-refreshed MongoDB data after bulk upload');
        }
      } else {
        setError(response.error || 'Failed to fetch data from server');
        setData([]);
      }
    });
  ...
  // Get unique column names (excluding _id)
    ...
    >
      <Paper
        ...
        sx={{
          ...
          borderRadius: 3,
          border: '2px solid',
          borderColor: connectionStatus === 'connected' ? '#10B981' : '#F59E0B',
          background: 'white',
      ...
      >
        ...
        {connectionStatus !== 'connected' && (
          ...
          >
            <Typography fontWeight={600}>
              {connectionStatus === 'connecting' ? 'Connecting to MongoDB server...' : 'MongoDB connection lost'}
            </Typography>
        ...
        <Box sx={{ textAlign: 'center', mb: 4 }}>
          <Stack direction="row" alignItems="center" justifyContent="center" spacing={2} sx={{ mb: 3 }}>
            <Box sx={{ 
              ...
              <StorageIcon sx={{ fontSize: { xs: 32, sm: 40 }, color: '#10B981' }} />
            </Box>
            <Box>
              ...
              <Typography variant="body1" color="text.secondary">
                Real-time NoSQL database synchronization with advanced features
              </Typography>
            </Box>
          </Stack>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems="center" justifyContent="center">
            ...
            <Tooltip title={connectionStatus === 'connected' ? "Refresh MongoDB data" : "Cannot refresh - Disconnected"}>
              <span>
                <IconButton 
        ...
        <Grid container spacing={3} sx={{ mb: 4 }}>
          <Grid item xs={12} sm={6} md={3}>
            <Card sx={{ 
              ...
              <CardContent sx={{ p: 3, textAlign: 'center' }}>
                <NumbersIcon sx={{ color: '#10B981', fontSize: 40, mb: 2 }} />
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Card sx={{ 
              ...
              <CardContent sx={{ p: 3, textAlign: 'center' }}>
                <ViewColumnIcon sx={{ color: '#3B82F6', fontSize: 40, mb: 2 }} />
        ...
        {error && (
          ...
          >
            <Typography fontWeight={600}>MongoDB Error</Typography>
        ...
        {(loading || isRefreshing) && (
          <Box sx={{ 
            ...
            />
            <Typography variant="h5" color="#047857" fontWeight={600} sx={{ mb: 1 }}>
              {isRefreshing ? 'Refreshing MongoDB Data...' : 'Loading MongoDB Database'}
            </Typography>
            <Typography variant="body1" color="#6B7280">
              {connectionStatus === 'connected' 
                ? 'Fetching documents from userschemas collection...' 
        ...
        {!loading && !error && data.length > 0 && (
          <Box sx={{ width: '100%' }}>
              ...
              <Typography variant="h5" sx={{ fontWeight: 700, color: '#047857' }}>
                ...
                <Typography component="span" variant="body1" color="#6B7280" sx={{ ml: 2 }}>
                  {filteredData.length === data.length 
                    ...
                    : `(${filteredData.length} of ${data.length} filtered documents)`}
                </Typography>
              </Typography>
              <Stack direction="row" spacing={2} alignItems="center">
                <Chip 
            ...
            {filteredData.length > 0 && (
              <Box sx={{ 
                ...
                <Typography variant="body2" color="#6B7280">
                  Showing {Math.min((page - 1) * rowsPerPage + 1, filteredData.length)} to {Math.min(page * rowsPerPage, filteredData.length)} of {filteredData.length} documents
                </Typography>
            ...
            <Typography variant="body2" color="#6B7280" sx={{ mt: 2, textAlign: 'center' }}>
              Click column headers to sort • Scroll to view all fields • Auto-updates when new data is inserted
            </Typography>
          </Box>
        )}
        ...
        {!loading && !error && data.length === 0 && (
          ...
          >
            <Typography variant="h6" fontWeight={600} color="#047857" sx={{ mb: 1 }}>
              MongoDB Collection Empty
            </Typography>
            <Typography variant="body1" color="#4B5563">
              The userschemas collection is empty. Submit data through the form to see documents here.
            </Typography>
        ...
        <Box sx={{ 
          ...
          <Typography variant="caption" color="#9CA3AF">
            Synchronized with MySQL • Updates when bulk uploads complete
          </Typography>
        </Box>
      </Paper>
    </Box>
